...
# Global counter for unique code IDs across all iterations
_global_code_counter = 0

root_folder = "/fsx/s3/chronos-o1/verl/checkpoints/verl_grpo/qwen3_32b_8nodes_bug_note_timeout_300_parallel_8/"
...
def execute_code_safely(code: str, historical_data: List[float], 
                        future_count: int, train_dir: str = "", val_dir: str = "", timeout: int = 300, code_id: int = 0) -> Tuple[float, Optional[np.ndarray], str]:
    """Execute generated code safely and return predictions"""

    code = clean_generated_code(code)
    
    os.makedirs(root_folder+"generated_codes", exist_ok=True)
    with open(root_folder+f"generated_codes/code_{code_id:04d}.py", 'w') as f:
        f.write(code)
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if os.path.exists(submission_path):
                ...
                shutil.copy2(submission_path, root_folder+f"generated_submission/submission_{code_id:04d}.csv")
                ...
                try:
                    ...
                    _log_error(code_id, error_msg, result.stderr)
...
@register("timeseriesparallel")
class TimeSeriesRewardParallelManager(AbstractRewardManager):
    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        # Collect all execution tasks with unique IDs
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            future_to_task = {executor.submit(execute_code_safely, task['response_str'], 
                                            task['historical_data'], task['future_count'], 
                                            task['train_dir'], task['val_dir'], 300, task['code_id']): task 
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    exe_reward, predictions, error_msg = future.result()
                    ...
                    if predictions is None:
                        accuracy_reward = 0.0
                        predictions = np.zeros(task['future_count'])
                    else:
                        try:
                            accuracy_reward = calculate_forecasting_reward(predictions, task['ground_truth'], task['historical_data'])
                        except Exception as e:
                            accuracy_reward = 0.0
                            predictions = np.zeros(task['future_count'])
                    ...
                    reward_tensor[i, task['valid_response_length'] - 1] = total_score
                    ...
                    reward_tensor[i, task['valid_response_length'] - 1] = 0.0
                ...
                if already_printed < self.num_examine:
                    ...
                    print("[prompt]", task['prompt_str'][:200] + "..." if len(task['prompt_str']) > 200 else task['prompt_str'])
                    print("[response]", task['response_str'][:200] + "..." if len(task['response_str']) > 200 else task['response_str'])
                    print("[execution_success]", execution_successes[-1])
                    print("[total_score]", reward_tensor[i, task['valid_response_length'] - 1].item())
